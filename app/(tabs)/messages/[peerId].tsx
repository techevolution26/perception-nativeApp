...
export default function ChatScreen() {
  ...
  const sendMutation=useMutation({mutationFn:(body:string)=>apiFetch<DisplayMessage>(`/api/conversations/${peerId}`,{method:"POST",body:{body}}),onMutate:async(body)=>{await qc.cancelQueries({queryKey:["messages",peerId]});const prev=qc.getQueryData<InfiniteData<MessagesPage>>(["messages",peerId]);qc.setQueryData<InfiniteData<MessagesPage>>(["messages",peerId],old=>old?{...old,pages:[{...old.pages[0],data:[...old.pages[0].data,{id:-Date.now(),body,from_user_id:me?.id??-1,to_user_id:peerId??-1,read_at:null,sending:true,created_at:new Date().toISOString()}]},...old.pages.slice(1)]}:old);return{prev}},onError:(_e,_b,c)=>{if(c?.prev)qc.setQueryData(["messages",peerId],c.prev);showToast({title:"Message not sent",message:"Your message could not be delivered. Please try again.",tone:"error"})},onSuccess:()=>{void playMessageSentSound();showToast({title:"Message sent",message:"Delivered to the conversation.",tone:"success"})},onSettled:()=>qc.invalidateQueries({queryKey:["messages",peerId]})});
  const editMutation=useMutation({mutationFn:({id,body}:{id:number;body:string})=>apiFetch<DisplayMessage>(`/api/messages/${id}`,{method:"PATCH",body:{body}}),onSuccess:()=>showToast({title:"Message edited",message:"Your change is now visible in the conversation.",tone:"success"}),onError:()=>showToast({title:"Message not edited",message:"Your change could not be saved. Please try again.",tone:"error"}),onSettled:()=>qc.invalidateQueries({queryKey:["messages",peerId]})});
