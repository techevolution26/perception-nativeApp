...
export default function ChatScreen() {
  ...
  const menuItems:ActionMenuItem[]=menuMessage?[{label:"Copy",icon:"copy",onPress:()=>void Clipboard.setStringAsync(menuMessage.body)},...(menuMessage.from_user_id===me?.id&&!menuMessage.sending&&!menuMessage.deleted_at?[{label:"Edit",icon:"edit-2",onPress:()=>{setEditText(menuMessage.body);setEditing(menuMessage);setMenuMessage(null)}},{label:"Recall for everyone",icon:"rotate-ccw",destructive:true,onPress:()=>deleteMutation.mutate(menuMessage.id)}]:[])]:[];
  return <KeyboardAvoidingView className="flex-1 bg-background" style={{paddingTop:insets.top}} behavior={Platform.OS==="ios"?"padding":"height"} keyboardVerticalOffset={Platform.OS==="ios"?insets.top+56:0}>
    ...
    {query.isLoading?<Spinner className="flex-1"/>:<FlatList ref={listRef} data={flat} keyExtractor={i=>String(i.id)} contentContainerClassName="gap-2 px-4 py-4" renderItem={({item})=>{const isMe=item.from_user_id===me?.id;return <View className={`flex-row ${isMe?"justify-end":"justify-start"}`}><Pressable onLongPress={()=>setMenuMessage(item)} delayLongPress={350} className="max-w-[82%]"><View className={`rounded-2xl px-4 py-2 ${isMe?"rounded-br-md bg-foreground":"rounded-bl-md border border-border-hairline bg-surface"} ${item.sending?"opacity-70":""}`}><Text className={`font-sans text-[15px] ${isMe?"text-background":"text-foreground"}`}>{item.body}</Text><Text className={`mt-1 font-mono text-[11px] ${isMe?"text-background/60":"text-foreground-subtle"}`}>{new Date(item.created_at).toLocaleTimeString([],{hour:"2-digit",minute:"2-digit"})}{item.sending?" · Sending…":item.edited_at?" · edited":""}</Text></View></Pressable></View>}}/>}
